...
export const clipSchema = z
  ...
  .superRefine((clip, ctx) => {
    if (
      ...
      Math.abs(clip.sourceEnd - clip.sourceStart - (clip.timelineEnd - clip.timelineStart)) >
        0.00001
    ) {
      ctx.addIssue({
        code: "custom",
        message: "Clip source and timeline durations must match and be positive",
      });
    }
  });
...
export const timelineSchema = z
  .object({
    ...
    hook: hookSchema,
    captions: captionTrackSchema.default(() => structuredClone(defaultCaptions)),
  })
  .superRefine((timeline, ctx) => {
    ...
    for (const track of timeline.tracks) {
      ...
      for (const clip of [...track.clips].sort((a, b) => a.timelineStart - b.timelineStart)) {
        ...
        ids.add(clip.id);
        if (clip.timelineStart < end - 0.00001)
          ctx.addIssue({ code: "custom", message: "Clips on a track cannot overlap" });
        end = clip.timelineEnd;
        duration = Math.max(duration, end);
      }
    }
    if (Math.abs(duration - timeline.duration) > 0.00001)
      ctx.addIssue({ code: "custom", message: "Incorrect timeline duration" });
  });
...
export function createTimeline(format: { width: number; height: number; fps: number }): Timeline {
  return {
    ...
    hook: structuredClone(defaultHook),
    captions: structuredClone(defaultCaptions),
  };
}
